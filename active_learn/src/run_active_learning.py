...
def load_task(data_folder, task, tag_column, document_separator, preprocess):
    X = {'train': [], 'test': []}
    y = {'train': [], 'test': []}
    tag_dictionary = Dictionary()
    tag_dictionary.add_item('<BOS>')
    tag_dictionary.add_item('<EOS>')
    ...
    for part in ('train', 'test'):
        ...
        if preprocess:
            column_name_map={0:'text', 1:'pos', tag_column:'ner'}
        else:
        ...
        corpus = ColumnDataset(
            ...
            comment_symbol=None,
            in_memory=True,
            document_separator_token='-DOCSTART-' if document_separator else None, # doc speractor for conll2003
        )

        for sent in corpus:
            tokens = [w.text for w in sent]
            # bypass doc start token
            if tokens[0] == '-DOCSTART-':
                continue
            if preprocess:
                X[part].append([(w.text, w.get_tag('pos').value) for w in sent])
            else:
...
def emulate_active_learning(train_tags, 
    ...
    for i in range(max_iterations):
        unlabeled_count = active_learner._active_learn_algorithm._train_dataset.get_unlabeled_entries()[0].shape[0]
        if unlabeled_count==0:
            print("training completed!")
            break
        try:
            print('Active learning iteration: #{}, unlabeled: #{}'.format(i,unlabeled_count))
...
def run_experiment(config):
    ...
    X_train, X_test, y_train, y_test, tag_dictionary = load_task(config.data.data_folder, 
                                                                 ...
                                                                 config.data.document_separator,
                                                                 preprocess)
    print('Done.')
    ...
    for repeat in range(config.n_repeats):
        ...
        model_name = config.model.model_type
        # reduce training time with minor training samples for dev purpose
